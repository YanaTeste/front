...
if (theme_get_setting('front_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('front_zen_tabs')) {
...
function front_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('front_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = front_id_safe('page-'. $path);
    $body_classes[] = front_id_safe('section-'. $section);
...
function front_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = front_id_safe('node-type-' . $vars['type']);
...
function front_preprocess_block(&$vars, $hook) {
    ...
    $classes = array('block');
    $classes[] = front_id_safe('block-' . $vars['block']->module);
    $classes[] = front_id_safe('block-' . $vars['block']->region);
    $classes[] = front_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('front_block_editing') && user_access('administer blocks')) {
...
function front_preprocess_comment(&$vars, $hook) {
...
function front_menu_item_link($link) {
...
function front_menu_local_tasks() {
  $output = '';
...
function front_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = front_id_safe(str_replace(' ', '_', strip_tags($link)));
